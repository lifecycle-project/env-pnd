...
rmarkdown::render(
  input = "./reports/31-may-21/env_pnd_desc_03_06_21.Rmd", 
  output_file = "env-pnd-desc-14-june-21.pdf", envir = globalenv())
```  
...
mutate(
  ...
  cohort_neat = fct_relevel(
    cohort_neat, 
    c("MoBa", "DNBC", "GENR", "ALSPAC", "NINFEA", "INMA"))) %>%
mutate(cohort_neat = factor(cohort_neat, ordered = TRUE))

}

desc_clean <- exposures.desc %>% map(cleanData)
...
hVPlot(var = "ndvi300", y_label = "NDVI") 
```
...
I'm waiting on the built environment variables from INMA. 
```{r lu-building}
hVPlot(
  var = "bdens300", 
...
# Single exposure - outcome associations
For most exposures there is very little evidence of an association. However
some evidence for a small negative association between NDVI and postnatal depression,
and a small positive association between PM2.5 and PND. However these are 
unadjusted models so will likely attenuate.

## NDVI
```{r}
single_reg.plotData %>%
filter(exposure %in% c("ndvi")) %>%
ggplot(aes(x = cohort_neat, y = est, colour = cohort_neat)) + 
  ...
  scale_colour_manual(values = palette_std) +
  coord_flip(
    ylim = c(0, 3),
    expand = FALSE,
    clip = "on")
...
## Distance to natural spaces
```{r}
single_reg.plotData %>%
filter(exposure %in% c("green_dist", "blue_dist")) %>%
ggplot(aes(x = cohort_neat, y = est, colour = cohort_neat)) + 
  geom_point(stat = "identity", size = 2) +
  geom_errorbar(aes(ymin = lowci, ymax = uppci), width=.2) +
  ylab("Odds ratio") +
...
```{r}
single_reg.plotData %>%
filter(exposure %in% c("no2", "pm25")) %>%
ggplot(aes(x = cohort_neat, y = est, colour = cohort_neat)) + 
  geom_point(stat = "identity") +
  geom_errorbar(aes(ymin = lowci, ymax = uppci), width=.1) +
  ylab("Odds ratio") +
  facet_wrap(~exposure, ncol = 1) +
  theme_std +
  theme_forest +
...
## Next steps
1. Decide how to harmonise DNBC PPD variable
2. Non-linear exposure-outcome associations
3. Meeting with IS Global collaborators
4. DAGs for each class of exposure
